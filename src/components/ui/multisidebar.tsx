...
const RIGHT_SIDEBAR_WIDTH = "48rem"
...
const MultiSidebarContext =
    React.createContext<MultiSidebarContextProps | null>(null)
...
function useSidebarBySide(side: SidebarSide = "left") {
    const { leftSidebar, rightSidebar } = useMultiSidebar()

    return side === "left" ? leftSidebar : rightSidebar
}
...
function MultiSidebarProvider({
    ...
    const setRightOpen = React.useCallback(
        (value: boolean | ((open: boolean) => boolean)) => {
            ...
            if (onRightOpenChange) {
                onRightOpenChange(openState)
            } else {
    ...
    const leftSidebar = React.useMemo<SidebarContextProps>(
        () => ({
    ...
    const rightSidebar = React.useMemo<SidebarContextProps>(
        () => ({
...
function Sidebar({
    ...
    const { isMobile, state, openMobile, setOpenMobile } = useSidebarBySide(side)

    const width =
        side === "right" ? "var(--right-sidebar-width)" : "var(--sidebar-width)"
        ...
        >
            <div
                className={cn(
                    ...
                    side === "right" && "w-(--right-sidebar-width)",
...
function SidebarTrigger({
    ...
    const { toggleSidebar } = useSidebarBySide(side)
...
function SidebarMenuButton({
    ...
    const { isMobile, state } = useSidebarBySide(side)
